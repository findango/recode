...
WebMidi.enable((err) => {
    ...
    input.addListener('controlchange', '1', (event) => {
        ...
        const binding = controlMap[controller.number];
        if (binding) {
            if (binding === 'thickness') {
                settings[binding] = 1 + (value / 127) * 100;
            } else if (binding === 'spacing') {
                settings[binding] = 0.5 + (value / 127) * 10;
            } else {
                settings[binding] = -5 + (value / 127) * 10;
            }
            pane.refresh();
        }
    });
